...
static u8 * format_class_trace (u8 * s, va_list * args)
{
  ...
  s = format (s, "CLASS: next_index %d, table %d, entry %d",
...
typedef enum {
#define _(sym,str) CLASS_ERROR_##sym,
  foreach_class_error
#undef _
  CLASS_N_ERROR,
} class_error_t;
...
class_action (vlib_main_t * vm,
              ...
              vlib_frame_t * frame)
{
	return class_inline (vm, node, frame, 1 /* is_ip4 */);
}
...
class_init (vlib_main_t * vm)
{
	  ...
	  vlib_node_add_next (vm, class_node.index, class_node.index);
  return 0;
}

VLIB_INIT_FUNCTION (class_init);
