...
typedef struct {
  ...
  u32 table_index;
  u32 entry_index;
} ip_classify2_trace_t;

/* packet trace format function */
static u8 * format_ip_classify2_trace (u8 * s, va_list * args)
{
  CLIB_UNUSED (vlib_main_t * vm) = va_arg (*args, vlib_main_t *);
  CLIB_UNUSED (vlib_node_t * node) = va_arg (*args, vlib_node_t *);
  ip_classify2_trace_t * t = va_arg (*args, ip_classify2_trace_t *);
  
  s = format (s, "IP_CLASSIFY2: next_index %d, table %d, entry %d",
...
typedef enum {
#define _(sym,str) IP_CLASSIFY2_ERROR_##sym,
  foreach_ip_classify2_error
#undef _
  IP_CLASSIFY2_N_ERROR,
} ip_classify2_error_t;

static char * ip_classify2_error_strings[] = {
...
static inline uword
ip_classify2_inline (vlib_main_t * vm,
                    ...
                    vlib_frame_t * frame, int is_ip4)
{
  ...
  vlib_node_increment_counter (vm, node->node_index, 
                               IP_CLASSIFY2_ERROR_MISS,
                               misses);
  vlib_node_increment_counter (vm, node->node_index, 
                               IP_CLASSIFY2_ERROR_HIT,
                               hits);
  vlib_node_increment_counter (vm, node->node_index, 
                               IP_CLASSIFY2_ERROR_CHAIN_HIT,
                               chain_hits);
...
VLIB_REGISTER_NODE (ip4_classify2_node) = {
  .function = ip4_classify2,
  .name = "ip4-classify2",
  .vector_size = sizeof (u32),
  .sibling_of = "ip4-lookup",
  .format_trace = format_ip_classify2_trace,
  .n_errors = ARRAY_LEN(ip_classify2_error_strings),
  .error_strings = ip_classify2_error_strings,

  .n_next_nodes = 0,
};

VLIB_NODE_FUNCTION_MULTIARCH (ip4_classify2_node, ip4_classify2)
